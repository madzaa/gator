...
func HandleLogin(s *state.State, cmd commands.Command) error {
	...
	if err != nil {
		log.Printf("HandleLogin error: unable to set user %v: %v", username, err)
...
func HandleRegistration(s *state.State, cmd commands.Command) error {
	...
	if len(args) == 0 || len(args) > 1 {
		err := errors.New("invalid username")
		log.Printf("HandleRegistration error: %v", err)
		return err
	}
	username := args[0]
	...
	if err != nil {
		log.Printf("HandleRegistration error: unable to create user %v: %v", username, err)
	...
	if err != nil {
		log.Printf("HandleRegistration error: unable to set user %v: %v", username, err)
		return fmt.Errorf("unable to set user %v: %v", username, err)
	}
	return nil
}
...
func HandleDeletion(s *state.State, cmd commands.Command) error {
	...
	if err != nil {
		log.Printf("HandleDeletion error: unable to delete users: %v", err)
...
func HandleListUsers(s *state.State, cmd commands.Command) error {
	...
	if err != nil {
		log.Printf("HandleListUsers error: unable to list users: %v", err)
...
func HandleAggregation(s *state.State, cmd commands.Command) error {
	...
	if err != nil {
		log.Printf("HandleAggregation error: %v", err)
		return err
	}
